...
function renderLectureContent(lecture, index) {
  ...
  header.innerHTML = `
                <h2>${lecture.title}</h2>
                <p>${lecture.audios.length} audio ${
    lecture.audios.length === 1 ? "file" : "files"
  } available</p>
            `;
...
function createAudioCard(audio, index) {
  ...
  audioPlayer.addEventListener("loadedmetadata", () => {
    ...
    document.getElementById(`duration-${currentLecture}-${index}`).textContent =
      duration;
  });
  ...
  audioPlayer.addEventListener("error", () => {
    ...
    errorMsg.textContent =
      "⚠️ Unable to load audio file. Please check the URL.";
...
function createDownloadButton(audio, audioPlayer) {
  ...
  downloadBtn.addEventListener("click", async () => {
    try {
      ...
      alert(
        'Unable to download the file. Please try right-clicking the audio player and selecting "Save audio as..."'
      );
    }
  });
